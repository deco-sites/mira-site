...
export default function Slide({ title = "Organização", content }: Props) {
  const renderSlideContent = (
    ...
    <div className="flex flex-col justify-center items-center gap-x-10 gap-col-4 mx-4 text-base-content">
      ...
      {Array(repeat).fill(0).map((_, index) => (
        <div
          key={index}
          className="flex justify-center items-center ml-[100%] gap-1 min-w-[142.86px] lg:min-w-[200px] min-h-[40px] lg:min-h-[58px] overflow-hidden"
        >
          <img
            className="w-[96px] lg:w-[134px] h-auto block"
            src={isDark ? srcDark || "" : srcLight || ""}
  ...
  return (
    <div className="flex flex-col items-center justify-center gap-8">
      ...
      <div class="relative w-full overflow-hidden h-28">
        <div class="animate-[sliding_60s_linear_infinite] absolute top-0 left-0 flex dark:hidden flex-nowrap">
          {Array(30).fill(0).map(() => slideContentDark)}
        </div>
        <div class="animate-[sliding_60s_linear_infinite] absolute top-0 left-0 hidden dark:flex flex-nowrap">
